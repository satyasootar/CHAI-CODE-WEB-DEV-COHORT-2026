...
export function generatePattern(n) {
  if (n <= 0 || typeof n !== "number" || !Number.isInteger(n)) return [];
  if (n === 1) return ["*"];
  const smaller = generatePattern(n - 1);          
  const ascending = smaller.slice(0, n - 1);       
  return [...ascending, "*".repeat(n), ...ascending.slice().reverse()];
}
