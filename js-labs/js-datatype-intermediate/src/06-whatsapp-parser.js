...
export function parseWhatsAppMessage(message) {
  ...
  const sender = message.substring(dashIndex + 2, colonIndex).trim()
  const messageExtract = message.substring(colonIndex + 2, message.length).trim()
  const wordCount = (messageExtract.split(" ").filter(t => t.length !==0 )).length


 function sentimentAnalysis(messageExtract){
  let text = messageExtract.toLowerCase()

  if(text.includes("😂") || text.includes(":)") || text.includes("haha")){
    return "funny"
  }else if(text.includes("❤") || text.includes("love") || text.includes("pyaar")){
    return "love"
  }else{
    return "neutral"
  }
 ...
 const sentiment = sentimentAnalysis(messageExtract)
  

  return { date, time, sender, text: messageExtract, wordCount, sentiment }
}
