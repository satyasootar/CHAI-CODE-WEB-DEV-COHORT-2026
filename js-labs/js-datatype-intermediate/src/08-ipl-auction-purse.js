...
export function iplAuctionSummary(team, players) {
  if( !team ||typeof team !== "object" ||team.purse == "null" || team.purse <= 0 || !Number.isFinite(team.purse)){
    return null
  }
  ...
  const totalSpent = players.reduce((accumulator, value)=> value.price + accumulator, 0)
  const remaining = team.purse - totalSpent;
  
  const playerCount = players.length

  const costliestPlayer = players.reduce((maxPlayer, currentPlayer) => {
  return currentPlayer.price > maxPlayer.price ? currentPlayer : maxPlayer;
});

    const cheapestPlayer = players.reduce((minPlayer, currentPlayer) => {
     return currentPlayer.price < minPlayer.price ? currentPlayer : minPlayer;
   });
...
const isOverBudget = totalSpent > team.purse
   return { teamName: team.name, totalSpent, remaining, playerCount, costliestPlayer, cheapestPlayer, averagePrice, byRole, isOverBudget }
}
